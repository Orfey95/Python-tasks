    ...
    for number_of_period in range(len(period_count)):
        ...
        for i in range(len(seconds_difference_list)):
            ...
            if sum >= dT:
                ...
                count_upstream_dict = {}
                for x in range(len(temp_upstream_list)):
                    if temp_upstream_list[x] != "":
                        count_upstream_dict[temp_upstream_list[x]] = temp_upstream_list.count(temp_upstream_list[x])
                #  Reverse sort of upstream statistic dictionary
                count_upstream_dict = dict(sorted(count_upstream_dict.items(), key=operator.itemgetter(1), reverse=True))
            ...
            if sum < dT and i == len(seconds_difference_list) - 1:
                ...
                for x in range(len(temp_upstream_list)):
                    if temp_upstream_list[x] != "":
                        count_upstream_dict[temp_upstream_list[x]] = temp_upstream_list.count(temp_upstream_list[x])
                #  Reverse sort of upstream statistic dictionary
                count_upstream_dict = dict(
                    sorted(count_upstream_dict.items(), key=operator.itemgetter(1), reverse=True))
                for key, value in count_upstream_dict.items():
                    print('Count: ', value, 'Upstream: ', key)
